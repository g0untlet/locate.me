...
description: BCE architecture patterns, BCE (Boundary-Control-Entity) structure, code conventions, and development guidelines for Quarkus microservices. Use when creating, scaffolding, generating, or writing code in a CA.micro project, when asking about BCE patterns, business component structure, package layout, CA.micro conventions, Quarkus microservice architecture, or when needing guidance on Java style, JAX-RS resources, testing strategy, JSON processing, or dependency injection in a CA.micro context. Triggers on "CA.micro", "BCE pattern", "business component", "boundary control entity", "CA.micro conventions", "Quarkus microservice structure", "create resource", "create entity", "create boundary", "add component", or requests to follow BCE architecture patterns.
metadata:
  authors:
    - g0untlet
  version: "0.0.1"
