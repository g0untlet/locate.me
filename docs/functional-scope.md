...
- `GET /positions/stats?adminKey={adminKey}`: Admin-only. Returns the number of stored positions per user as a JSON array of `{"userId": ..., "locations": ...}` objects, ordered by `userId`. Requires the `adminKey` query parameter to match the configured `admin.key` (overridable via the `ADMIN_KEY` environment variable); a missing or mismatched key returns `401 Unauthorized`. Not rate-limited.
- `GET /places/stats?adminKey={adminKey}`: Admin-only. Returns the number of entries in the places cache as `{"count": ...}`. Requires the `adminKey` query parameter (same check as above); a missing or mismatched key returns `401 Unauthorized`. Not rate-limited.
