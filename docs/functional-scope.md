...
- The Locate view can additionally show a short hourly forecast for the next few hours (configurable via `weather.forecast-hours`, default 4). Each slice shows the local time, a condition icon (day/night-aware), the temperature (°C, rounded), the UV index (rounded) and the precipitation probability.
...
- Each cached place records the origin of the fetch that produced it — the user's request coordinates (`fetch_lat`/`fetch_lon`) — so a coverage-aware cache-read algorithm can be built later. This metadata is internal and never returned to the client.
...
- Fetching runs a GPS accuracy loop; once a fix is found, the view splits into two steps:
  - **Chooser step:** shows the current weather (temperature, condition, UV-Index), a selectable **Resolved Address** row (with elevation; always reserves two lines so the row height is stable) and a **"Places around me"** list of up to the 20 nearest POIs (configurable via `aroundme.max-places`; category icon + name + distance in m/km). The resolved address is selected by default. The user either keeps it or taps one of the place rows to adopt it as the location label — a chosen place is shown as `Name, <street> <houseNumber>`. A "Continue" button proceeds to the save step. The fetch button now reads "Refresh" and re-runs the GPS + preview fetch from the current coordinates. The places card stretches to fill the remaining space and the "Continue" button stays pinned at the bottom of the view; the places list scrolls internally only when its rows overflow, so the page itself never scrolls. The weather box is tappable: it expands a compact forecast grid (one column per forecast hour; rows TEMP / UV / RAIN — or SNOW when every hour is a snow code) and collapses again on a second tap; condition icons reflect day/night (sun/moon, sun+cloud, cloud+moon, cloud).
  ...
  - **Save step:** shows the collapsible "Tag & Comment" section, the weather again, a **LOCATION** row with the chosen location (resolved address or adopted place) and the elevation (from the fetched weather data), and an OpenStreetMap snippet of the position that will be saved. When an adopted place carries contact data, a separate compact **PLACE DETAILS** box appears below the location/elevation card: phone (tap to call via `tel:` on Android/iOS), website (external link, displayed as its hostname), opening hours and wheelchair accessibility, each with an icon; the box stays hidden when none of these fields is present. On this step the redundant top "Refresh" button and the "Preview from …" status line are hidden — the "Back" button returns to the chooser where a fresh fetch is available. "Save Location" persists the position; a "Back" button (same width, left of Save) returns to the chooser step without reloading data from the backend.
...
- `GET /positions/current?userId={userId}&lat={lat}&lon={lon}`: Resolves the address (Nominatim) and weather/UV/elevation plus a short hourly forecast (Open-Meteo) for the given coordinates and returns a preview without persisting it. This is used by the Locate view to fetch the current location before saving.
...
Position responses include the persisted weather-related fields `temperature`, `weatherCode`, `uvIndex`, and `elevation`. The `/positions/current` preview additionally returns a response-only `isDay` flag for the current conditions and a `forecast` array with the next `weather.forecast-hours` future hours (the current hour is skipped); each slice carries `time`, `temperature`, `weatherCode`, `uvIndex`, `precipitationProbability` and `isDay`. Neither `isDay` nor `forecast` is persisted, and they are not returned by the other endpoints. No new endpoints were introduced for UV-Index and elevation; they are persisted and returned by the existing endpoints above.
...
- The `/positions/current` preview's `forecast` array and `isDay` flag are display-only and are not attributes of the persisted `Position`.
...
|---------|------|-------------|
| 0.4.2 | 2026-09-21 | Locate save step: removed the obsolete top "Refresh" button and the "Preview from …" status line from the pre-save (saver) view — the "Back" button returns to the chooser where a fresh fetch is available (the status line still appears for save progress/errors). When an adopted place provides contact data, a new separate compact **PLACE DETAILS** card is shown below the location/elevation card with icons: phone (tap-to-call `tel:` link), website (external link, hostname label), opening hours and wheelchair accessibility; it is hidden when none of the four fields is present. Only the content above the action row scrolls (`.saver-scroll`), so "Back" and "Save Location" stay visible when the "Tag & Comment" accordion is expanded. The chooser's places card now stretches to fill the remaining space and the "Continue" button is pinned at the bottom (the list scrolls internally only on overflow). New `label.placeDetails` and `placeInfo.*` keys in en/de/es. |
