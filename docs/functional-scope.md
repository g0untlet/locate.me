...
- **Status:** fully integrated into the Locate view (0.4.0): the up-to-10 nearest places (configurable via `aroundme.max-places`) are shown with their distance and direction (e.g. `301 m NW`); the user can adopt a place as the location — its coordinates and label ("Name, street houseNumber") are then saved.
